...
export default class AspectOfWordMouth extends React.Component {
    ...
    handleBrandSelect = (eventKey, event) => {
        ...
        if (this.state.aspectTitle != "请选择需要查看的Aspect") {
            let product_pinyin = this.props.product_pinyin;
            let url = 'http://110.64.66.203:5000/' + product_pinyin + '/aspect_trend/' + eventKey + '/' + this.state.aspectKey;
            // console.log(url);
            $.get(url)
                .then((response) => {
                    // console.log(response.data);
    ...
    handleAspectSelect = (eventKey, event) => {
        ...
        if (this.state.brandTitle != "请选择需要查看的品牌") {
            // alert(aspect_name);
            let product_pinyin = this.props.product_pinyin;
            let url = 'http://110.64.66.203:5000/' + product_pinyin + '/aspect_trend/' + this.state.brandKey + '/' + String(eventKey);
            // console.log(url);
            $.get(url)
                .then((response) => {
                    // console.log(response.data);
